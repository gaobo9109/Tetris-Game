...
public class PlayerSkeleton {
	...
	{
		features[0].setFeatureWeight(38.28830094700409);
		features[1].setFeatureWeight(-569.2945026095888);
		features[2].setFeatureWeight(-1238.7869002491661);
		features[3].setFeatureWeight(-192.21258021366077);
		features[4].setFeatureWeight(5684.271090071902);
		features[5].setFeatureWeight(-73.12397872562666);
		features[6].setFeatureWeight(-108.72791357401769);
		features[7].setFeatureWeight(-5912.355586437261);
		features[8].setFeatureWeight(-6909.586722215406);
		features[9].setFeatureWeight(-9418.49084366277);
		features[10].setFeatureWeight(7.954570036023128);
		features[11].setFeatureWeight(-3758.2746981600026);
		features[12].setFeatureWeight(-72.39903769751653);
	}
	...
	public int pickMove(State s, int[][] legalMoves) {
		...
		for (int i = 0; i < legalMoves.length; i++) {
			...
			double score = heuristic.calculateHeuristicScore(s, ns);
...
//			s.makeMove(p.pickMove(s, s.legalMoves(), ns, hs));
	...
	public static void main(String[] args) throws IOException {
		// State s = new State();
		// NextState ns = new NextState();
		Heuristic hs = (new PlayerSkeleton()).new Heuristic();
//		BufferedReader fr = new BufferedReader(new FileReader("heuristic.txt"));
	...
	/**
	 * Generates next state of the game. Useful for calculating score of next
	 * state
	 *
	 * @author Advay Pal, Chang Chu-Ming, Gao Bo, Herbert Ilhan Tanujaya, Varun
	 *         Gupta
...
//
	...
	public class Heuristic implements Comparable<Heuristic> {
		...
		protected Feature[] features = new Feature[] { // Define included
														// features
...
//	}
	...
	public class AltitudeDiff extends Feature {
		...
		public void updateScore(NextState s, int row, int col) {
			int[] top = s.getTop();
			if (top[col] > maxHeight)
				maxHeight = top[col];
			else if (top[col] < minHeight)
				minHeight = top[col];
	...
	/*
	 * Counting unoccupied -> occupied and occupied -> unoccupied transition
	 * within a column
	 */
	...
	public class ColTransition extends Feature {
		...
		public void updateScore(NextState s, int row, int col) {
			...
			if (row == 0 && field[row][col] == 0)
				count++;
			if (field[row][col] == 0 && field[row + 1][col] != 0)
				count++;
			else if (field[row][col] != 0 && field[row + 1][col] == 0)
				count++;
	...
	public abstract class Feature {
		...
		/**
		 * Resets. Should be overridden with proper implementation of scoring
		 */
	...
	/**
	 * This heuristic calculates the number of holes. A column has a hole if it
	 * is lower than the highest column.
	 */
	...
	/**
	 * This heuristic calculates the number of wells. A well is defined as a
	 * column that is lower than its adjacent columns.
	 */
	...
	public class RowTransition extends Feature {
		...
		public void updateScore(NextState s, int row, int col) {
			...
			if (col == 0 && field[row][col] == 0)
				count++;
			else if (col == cols - 1 && field[row][col] == 0)
				count++;

			if (col != cols - 1) {
				if (field[row][col] == 0 && field[row][col + 1] != 0)
					count++;
				else if (field[row][col] != 0 && field[row][col + 1] == 0)
					count++;
			}
